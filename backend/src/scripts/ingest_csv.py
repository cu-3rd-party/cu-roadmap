...
from src.domain.models import Base, Course, Major, CourseDependency, CourseType, CourseCategory, DependencyType
    ...
    async with async_session() as session:
        ...
        with open('majors.csv', mode='r', encoding='utf-8') as f:
            ...
            for row in reader:
                ...
                major = Major(
                    id=uid,
                    title=row['title'],
                    school="Tech" if "AI" in row['title'] or "Software" in row['title'] else "Business"
                )
                session.add(major)
        ...
        with open('course_dependencies.csv', mode='r', encoding='utf-8') as f:
            ...
            for row in reader:
                ...
                if row['course_id'] in course_map and row['required_course_id'] in course_map:
