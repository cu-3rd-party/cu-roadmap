...
export function CoursesPage({ passedIds, setPassedIds }: CoursesPageProps) {
  const [courses, setCourses] = useState<{ id: string; title: string; category: string; workload: number }[]>([]);
        ...
        {courses.map((c) => (
          <div
            ...
            style={{
              ...
              borderColor: passedIds.includes(c.id) ? "var(--color-primary)" : "var(--color-border)",
            }}
          ...
          >
            <div className="h-36 relative" style={{ backgroundColor: getCategoryColor(c.category) }}>
              ...
              <div className="absolute bottom-3 right-3 w-8 h-8 rounded-full flex items-center justify-center shadow-md" style={{ backgroundColor: "var(--color-bg-main)" }}>
                <Search size={14} color={getCategoryColor(c.category)} />
              </div>
            </div>
            <div className="p-4">
              <div className="font-bold text-base mb-1 leading-snug" style={{ color: "var(--color-text-main)" }}>
                {c.title}
              </div>
            ...
            {passedIds.includes(c.id) && (
              <div className="absolute top-3 left-3 px-2 py-1 rounded text-xs font-extrabold" style={{ backgroundColor: "rgba(0,0,0,0.7)", color: "white" }}>
                ПРОЙДЕНО
              </div>
            )}
          </div>
