  ...
  return (
    <div className="flex flex-col w-full">
      ...
      <p className="mb-6" style={{ color: "var(--color-text-muted)" }}>
        Выберите курс, который вы хотите пройти в будущем, и система построит кратчайший путь до него.
      </p>
        ...
        <div className="flex flex-col gap-8 mt-10">
          {roadmap.map((sem, idx: number) => (
            <div
              ...
              style={{ backgroundColor: "var(--color-bg-hover)", borderWidth: 1, borderStyle: "solid", borderColor: "var(--color-border)" }}
            >
              <div className="flex justify-between items-center mb-4">
                ...
                {sem.total_load && (
                  <span className="text-xs px-2 py-1 rounded font-semibold" style={{ backgroundColor: "var(--color-bg-main)", color: "var(--color-text-muted)" }}>
                    Нагрузка: {sem.total_load.toFixed(1)}
                  </span>
                )}
              </div>
              {sem.error && <div className="text-sm mb-2" style={{ color: "#ef4444" }}>⚠️ {sem.error}</div>}
              {sem.status && <div className="text-sm mb-2" style={{ color: "var(--color-text-muted)" }}>{sem.status}</div>}
              <div className="grid gap-3" style={{ gridTemplateColumns: "repeat(auto-fill, minmax(240px, 1fr))" }}>
                {(sem.courses || []).map((c) => (
                  <div key={c.id} className="rounded-xl p-3 shadow-sm" style={{ backgroundColor: "var(--color-bg-main)", borderWidth: 1, borderStyle: "solid", borderColor: "var(--color-border)" }}>
                    <strong className="font-semibold" style={{ color: "var(--color-text-main)" }}>{c.title}</strong>
                    <div className="text-xs mt-1" style={{ color: "var(--color-text-muted)" }}>{c.workload} к.</div>
                  </div>
                ))}
              </div>
            </div>
