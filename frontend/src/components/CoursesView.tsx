...
export function CoursesView({passedIds, setPassedIds}: any) {
    ...
    return (
        <div className="flex flex-col w-full">
            <div className="text-xs text-gray-500 uppercase font-semibold tracking-wide mb-2">Траектория &gt; Каталог курсов</div>
            <h1 className="text-3xl font-extrabold mb-8 text-gray-900 tracking-tight">Курсы и навыки</h1>

            <div className="grid gap-6 w-full" style={{gridTemplateColumns: 'repeat(auto-fill, minmax(280px, 1fr))'}}>
                {filtered.map(c => (
                    <div key={c.id} className={`bg-white rounded-2xl overflow-hidden cursor-pointer border transition-all hover:-translate-y-1 hover:shadow-lg select-none ${passedIds.includes(c.id) ? 'border-primary border-2' : 'border-gray-100'}`}
                         onClick={() => toggleCourse(c.id)}>
                        <div className="h-36 relative" style={{backgroundColor: getCategoryColor(c.category)}}>
                        ...
                        <div className="p-4">
                            <div className="font-bold text-base mb-1 text-gray-900 leading-snug">{c.title}</div>
                            <div className="text-xs font-medium text-gray-500">{c.category} • {c.workload} к.</div>
                        </div>
                        {passedIds.includes(c.id) && <div className="absolute top-3 left-3 bg-black/70 text-white px-2 py-1 rounded text-xs font-extrabold">ПРОЙДЕНО</div>}
                    </div>
                ))}
            </div>
        </div>
